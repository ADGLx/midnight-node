...
run_ledger_state_generation() {
    ...
    if [[ "$network" == "mainnet" ]]; then
        echo -e "${BOLD}Command to execute:${NC}"
        echo -e "  ${CYAN}earth -P +$earthly_target${NC}"
        earthly_cmd=(earth -P "+$earthly_target")
    else
        echo -e "${BOLD}Command to execute:${NC}"
        echo -e "  ${CYAN}earth -P +$earthly_target --RNG_SEED=$rng_seed${NC}"
        earthly_cmd=(earth -P "+$earthly_target" "--RNG_SEED=$rng_seed")
    fi
    echo ""
