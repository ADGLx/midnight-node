...
impl<D: DB> Ledger<D> {
	...
	pub(crate) fn post_block_update(
		...
		log::debug!(target: LOG_TARGET, "post_block_update - tblock: {:?}, block fullness: {:?} block context: {:?}", Timestamp::from_secs(block_context.tblock), block_fullness, block_context);
		let next_state = sp
			.state
