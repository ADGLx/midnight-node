...
test-toolkit:
    ...
    # (These tests depend on .mn files generated by Midnight Node Toolkit or Node Toolkit (JS) npm packages)
    RUN MIDNIGHT_LEDGER_EXPERIMENTAL=1 cargo llvm-cov nextest --profile ci --release --locked \
        -E 'package(midnight-node-toolkit) | test(/^tests::test_get_contract_state$/) | test(/^tests::test_send_mn_transaction$/) | test(/^tests::test_validation_works$/)'
