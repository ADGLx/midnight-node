...
prep-no-copy:
    ...
    FROM midnightntwrk/midnight-node-ci-bazel:${RUST_VERSION}-$NATIVEARCH
...
# test (cargo) - superseded by Bazel test: target below.
# Kept as reference for the cargo flags used on main (SKIP_WASM_BUILD, opt-level=2, debuginfo=1).
