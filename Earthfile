...
test:
    ...
    RUN MIDNIGHT_LEDGER_EXPERIMENTAL=1 cargo llvm-cov nextest --profile ci --release --workspace --locked \
        ...
        -E 'not (test(pallet_midnight::tests::test_get_contract_state) | test(pallet_midnight::tests::test_send_mn_transaction) | test(pallet_midnight::tests::test_validation_works))'
...
test-toolkit:
    ...
    # Run midnight-node-toolkit tests AND pallet-midnight fixture tests
    # (These tests depend on .mn files generated by toolkit or toolkit-js npm packages)
    RUN MIDNIGHT_LEDGER_EXPERIMENTAL=1 cargo llvm-cov nextest --profile ci --release --locked \
        -E 'package(midnight-node-toolkit) | test(pallet_midnight::tests::test_get_contract_state) | test(pallet_midnight::tests::test_send_mn_transaction) | test(pallet_midnight::tests::test_validation_works)'
