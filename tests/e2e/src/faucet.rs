...
//! ## Sizing
//!
//! - **N (worker count)**: defaults to 4 (matches CI's `--test-threads=4`). Override with
//!   `E2E_FAUCET_WORKERS=<n>` to right-size for the actual test parallelism — a single-test
//!   run only needs 1, a 16-thread run wants 16. Larger N reduces queueing but each worker
//!   needs to be large enough to handle several requests.
...
fn worker_count() -> usize {
    ...
    *N.get_or_init(|| {
        ...
        println!("FaucetManager: using {} worker(s) (override with {})", n, WORKERS_ENV_KEY);
        n
    })
}
...
impl FaucetManager {
    ...
    async fn prime(faucet: &CardanoClient, utxos: &[OgmiosUtxo]) -> Vec<OgmiosUtxo> {
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    for (id, idx) in &ghosts {
                        println!("  ghost: {}#{}", hex::encode(id), idx);
                    }
        ...
        println!("FaucetManager: prime tx submitted: {}", prime_tx_id_hex);
        ...
        println!("FaucetManager: priming complete:");
