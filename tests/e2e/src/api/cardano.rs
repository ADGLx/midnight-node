...
impl CardanoClient {
	...
	fn print_addresses(wallet: &Wallet, network_info: &NetworkInfo) {
		...
		println!("Stake public key hash: {}", stake_cred.to_keyhash().unwrap().to_hex());
	}

	fn wallet_for_funded(cli_skey: &str) -> Wallet {
		let cli_hex = cli_skey.strip_prefix("5820").unwrap_or(cli_skey).to_string();
	...
	fn derive_stake_signing_key_from_mnemonic(wallet: &Wallet) -> Result<PrivateKey, WError> {
		...
		// m / 1852' / 1815' / 0'
		root = root.derive(1852 | 0x8000_0000).derive(1815 | 0x8000_0000).derive(0x8000_0000);
	...
	async fn ogmios_request(
		...
		let client = client_for_url(&config.base_url, Duration::from_secs(config.timeout_seconds))
			.await
		...
		match req {
			...
			OgmiosRequest::QueryUtxo { address } => {
				let utxos = client.query_utxos(&[address]).await.expect("Failed to get utxos");
	...
	pub async fn utxos(&self) -> Vec<OgmiosUtxo> {
		let request = OgmiosRequest::QueryUtxo { address: self.address_as_bech32() };
	...
	pub async fn register(
		...
		let response = Self::ogmios_request(&self.ogmios_settings, request).await.unwrap();
		match response {
			OgmiosResponse::SubmitTx(res) => Ok(res),
			_ => Err(OgmiosClientError::RequestError("Unexpected response type".into())),
		}
	}
	...
	pub async fn deregister(
		...
		let signed_tx = self.wallet.sign_tx(&tx_builder.tx_hex()).expect("Failed to sign tx");
	...
	pub async fn mint_tokens(
		...
		let request = OgmiosRequest::QueryUtxo { address: payment_addr.clone() };
		let response = Self::ogmios_request(&self.ogmios_settings, request).await.unwrap();
	...
	pub async fn send(
		...
		let payment_addr = self.address_as_bech32();
		println!("Sending assets from {} to address: {}", payment_addr, tx_out_addr);
	...
	pub async fn find_utxo_by_tx_id(&self, address: &str, tx_id_hex: String) -> Option<OgmiosUtxo> {
		...
		let request = OgmiosRequest::QueryUtxo { address: address.to_string() };

		for _ in 0..MAX_ATTEMPTS {
			let response =
				Self::ogmios_request(&self.ogmios_settings, request.clone()).await.unwrap();
			let utxos = match response {
				OgmiosResponse::QueryUtxo(utxos) => utxos,
				_ => vec![],
			};
	...
	pub fn reward_address_bytes(&self) -> [u8; 29] {
		let cred = self.wallet.addresses.base_address.as_ref().unwrap().stake_cred();
