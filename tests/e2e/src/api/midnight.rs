...
impl MidnightClient {
	...
	pub async fn subscribe_to_federated_authority_events(
		...
		let check_block_events = |events: subxt::events::Events<SubstrateConfig>,
			...
			if let Some(event) = events.find::<TechnicalCommitteeMembersReset>().flatten().next() {
				println!(
		...
		let finalized_block = self.online_client.blocks().at(finalized_hash.hash()).await?;
		...
		for block_num in 1..=current_finalized {
			let block_hash: H256 =
				self.rpc_client.request("chain_getBlockHash", rpc_params![block_num]).await?;
	...
	pub async fn get_d_parameter(&self) -> Result<DParameterResponse, Box<dyn std::error::Error>> {
		let response: DParameterResponse =
			self.rpc_client.request("systemParameters_getDParameter", rpc_params![]).await?;
	...
	pub async fn get_block_hash_at_height(
		...
		let block_hash: Option<H256> =
			self.rpc_client.request("chain_getBlockHash", rpc_params![block_number]).await?;
	...
	pub async fn get_ariadne_parameters(
		...
		let response: AriadneParametersResponse = match d_parameter_at {
			...
			None => {
				self.rpc_client
					.request("systemParameters_getAriadneParameters", rpc_params![epoch_number])
					.await?
			},
		};
	...
	pub async fn get_sidechain_status(
		...
		let response: SidechainStatusResponse =
			self.rpc_client.request("sidechain_getStatus", rpc_params![]).await?;
		...
		while let Some(status) = progress.next().await {
			match status? {
				subxt::tx::TxStatus::InBestBlock(block_info) => {
					println!("Transaction included in best block: {:?}", block_info.block_hash());
					return Ok(());
				},
				subxt::tx::TxStatus::InFinalizedBlock(block_info) => {
					println!("Transaction finalized in block: {:?}", block_info.block_hash());
					return Ok(());
				},
