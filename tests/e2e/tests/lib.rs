...
async fn register_for_dust_production() {
	...
	let register_tx_id = cardano_client
		...
		.transaction
		.id;
	println!("Registration transaction submitted with hash: {}", hex::encode(register_tx_id));

	let reward_address = cardano_client.reward_address_bytes();
	let dust_address: Vec<u8> =
...
async fn verify_governance_contracts_and_validate_membership_reset() {
	...
	println!("✓ Technical Authority Forever contract verified at {}", tech_auth_address);
	...
	let events_result =
		timeout(Duration::from_secs(30), midnight_client.subscribe_to_federated_authority_events())
			.await;
...
async fn verify_federated_ops_contract_deployment() {
	...
	println!("✓ Federated Operators Forever contract verified at {}", federated_ops_address);
...
async fn register_2_cardano_same_dust_address_production() {
	...
	let result = dust_balance::execute(args).await.expect("dust-balance error");
	...
	if let DustBalanceResult::Json(DustBalanceJson { generation_infos, .. }) = &result {
		let actual: HashSet<String> =
			generation_infos.iter().map(|p| p.dust_output.backing_night.clone()).collect();
...
async fn cnight_produces_dust() {
	...
	let midnight_wallet_seed = MidnightClient::new_seed();
	println!("Midnight wallet seed: {}", hex::encode(midnight_wallet_seed.as_bytes()));
	let dust_hex = MidnightClient::new_dust_hex(midnight_wallet_seed);
	...
	match cardano_client
		.find_utxo_by_tx_id(&cardano_client.address_as_bech32(), hex::encode(register_tx_id))
		.await
	{
		Some(_) => (),
	...
	let result2 = dust_balance::execute(args2).await.expect("dust-balance error");
...
async fn deregister_from_dust_production() {
	...
	let deregister_tx = cardano_client
		...
		.transaction
		.id;
	println!("Deregistration transaction submitted with hash: {}", hex::encode(deregister_tx));
...
async fn alice_cannot_deregister_bob() {
	...
	let faucet = global_faucet_manager().await;
	let alice_collateral = faucet.request_tokens(&alice.address_as_bech32(), 5_000_000).await;
	let deregister_tx_in = faucet.request_tokens(&alice.address_as_bech32(), 10_000_000).await;
...
async fn removing_excessive_registrations() {
	...
	let utxos = cardano_client.utxos().await;
	assert_eq!(utxos.len(), 4, "New wallet should have exactly two UTXOs after funding");
	...
	println!("Matching second MappingAdded event found: {:?}", second_mapping_added.unwrap());
	...
	let utxo_owner_hex = hex::encode(utxo_owner.unwrap().0.0);
	println!("UTXO owner in hex: {:?}", utxo_owner_hex);
	assert_eq!(utxo_owner_hex, second_dust_hex, "UTXO owner does not match DUST address");
}
...
async fn create_hundred_registrations() {
	...
	for i in 0..101 {
		...
		tx_in = cardano_client
			.find_utxo_by_tx_id(&cardano_client.address_as_bech32(), hex::encode(register_tx_id[i]))
			.await
	...
	for i in 0..100 {
		...
		println!("Deregistration transaction submitted with hash: {}", hex::encode(deregister_tx));
		tx_in = cardano_client
			.find_utxo_by_tx_id(&cardano_client.address_as_bech32(), hex::encode(deregister_tx))
			.await
			.expect("Failed to find UTXO for next registration");
...
async fn ddos_batch_attack_all_rejected() {
	...
	println!("✓ PR367-TC-0003-06 PASSED: All {} attack transactions rejected", total_attacks);
}
...
async fn register_twice_with_same_cardano_address() {
	...
	let register_tx_id2 = cardano_client
		...
		.transaction
		.id;
	println!("Registration transaction submitted with hash: {}", hex::encode(register_tx_id2));
...
async fn query_d_parameter_at_historical_block() {
	...
	println!("Genesis block hash: 0x{}", hex::encode(genesis_block_hash.as_bytes()));
	...
	println!("Current block hash: 0x{}", hex::encode(current_block_hash.as_bytes()));
	...
	println!("✓ Invalid block hash correctly rejected: {}", invalid_query_result.unwrap_err());
...
async fn deregister_first_mapping() {
	...
	let result3 = dust_balance::execute(args3).await.expect("dust-balance error");
	...
	let result4 = dust_balance::execute(args4).await.expect("dust-balance error");
...
async fn produce_dust_from_tokens_owned_before_registration() {
	...
	println!("Rotated cNIGHT UTXO: {}", &hex::encode(&cnight_utxo_new.transaction.id));
...
async fn get_ariadne_parameters_returns_valid_structure() {
	...
	println!(
		...
		ariadne_params.permissioned_candidates.as_ref().map(|c| c.len())
	);
...
async fn d_parameter_from_pallet_matches_config() {
	...
	let d_param = midnight_client.get_d_parameter().await.expect("Failed to get D-Parameter");
...
async fn permissioned_candidates_aiken_format() {
	...
	if let Some(candidates) = &ariadne_params.permissioned_candidates {
		...
		for (i, candidate) in candidates.iter().enumerate() {
			...
			println!("  Candidate {}: sidechain={}, has_keys={}", i, has_sidechain_key, has_keys);

			assert!(has_sidechain_key, "Candidate {} should have sidechain public key", i);
			assert!(has_keys, "Candidate {} should have keys object with aura and gran entries", i);
		}
...
async fn authority_selection_uses_aiken_candidates() {
	...
	println!("Permissioned candidates from Aiken contracts: {}", candidates.len());

	assert!(!candidates.is_empty(), "Expected at least one permissioned candidate");
	...
	for (i, candidate) in candidates.iter().enumerate() {
		let keys = candidate.get("keys").expect(&format!("Candidate {} missing 'keys' field", i));

		// Validate AURA key
		let aura_key = keys.get("aura").expect(&format!("Candidate {} missing 'aura' key", i));
		...
		let grandpa_key = keys.get("gran").expect(&format!("Candidate {} missing 'gran' key", i));
		let grandpa_str = grandpa_key.as_str().unwrap_or("");
		assert!(!grandpa_str.is_empty(), "Candidate {} has empty GRANDPA key", i);
		...
		assert!(!sidechain_str.is_empty(), "Candidate {} has empty sidechainPublicKey", i);
