...
impl BlockDataSourceImpl {
	...
	/// Tests if out Cardano view matches Praos requirements regarding chain density.
	...
	pub async fn is_cardano_ok(&self) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
		...
		let stable = self.get_latest_stable_block_for(timestamp).await?;
		log::info!("is_cardano_ok timestamp: {timestamp}, stable: {stable:?}");
		Ok(stable.is_some())
	}
}
...
impl BlockDataSourceImpl {
	...
	/// This is heuristic (unlike [Self::is_stable_block_time_valid]) of judging the latest
	...
	fn is_latest_block_time_valid(&self, block: &MainchainBlock) -> bool {
		...
		log::info!("current time secs: {current_time_secs}");
	...
	async fn get_stable_block_by_hash(
		...
		match self.get_stable_block_by_hash_from_db(hash, reference_timestamp).await {
			...
			| Err(StableBlockByHashError::TimestampOutOfRange { block, .. }) => {
				Ok(StableBlockByHashResult::BlockNotStable { info: From::from(block) })
			},
			Err(err) => {
