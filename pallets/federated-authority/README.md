...
The `federated_authority` [pallet](../../GLOSSARY.md#pallet) implements a [federated authority](../../GLOSSARY.md#federated-authority) system where multiple distinct on-chain authority bodies must collectively approve a motion before it can be executed with elevated `Root` privileges. This creates a final checkpoint requiring consensus from multiple governance groups before any critical action can be executed.

The [pallet](../../GLOSSARY.md#pallet) is configurable to define:
...
| `runtime-benchmarks` | No | [Benchmarking](../../GLOSSARY.md#benchmarking) support |
...
- On the first approval, the [pallet](../../GLOSSARY.md#pallet) creates a motion entry in storage with an expiration period
...
The `motion_close` [extrinsic](../../GLOSSARY.md#extrinsic) can be called by anyone to finalize a motion. A motion can only be closed if it has either been approved by all required bodies or has expired.

### 4. Revoking an Approval

The `motion_revoke` [extrinsic](../../GLOSSARY.md#extrinsic) allows an authority body to withdraw its approval before execution. If all approvals are revoked, the motion is immediately removed from storage.
...
- `frame-support` / `frame-system` - [FRAME](../../GLOSSARY.md#frame-framework-for-runtime-aggregation-of-modularized-entities) primitives
...
- [GLOSSARY - Federated Authority](../../GLOSSARY.md#federated-authority) - Term definition
