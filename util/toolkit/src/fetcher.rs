...
pub async fn read_blocks_from_cache(
	...
	log::info!("[perf] get_highest_verified_block took {:?}", t.elapsed());
	...
	let mut blocks: Vec<_> = fetch_storage
		...
		.collect();
	log::info!("[perf] get_block_data_range: {} blocks in {:?}", blocks.len(), t.elapsed());
	...
	log::info!("[perf] last_block_time fixup: {} blocks in {:?}", blocks.len(), t.elapsed());
...
pub async fn fetch_from_rpc(
	...
	log::info!("[perf] fetch_from_rpc RPC pipeline took {:?}", t_rpc_total.elapsed());
	...
	let t = std::time::Instant::now();
	let blocks = read_blocks_from_cache(chain_id, fetch_storage).await?;
	log::info!("[perf] fetch_from_rpc read_blocks_from_cache took {:?}", t.elapsed());
