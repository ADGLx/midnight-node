...
use crate::{
	builder::{
		...
		ContractDeployInfo, DeserializedTransactionsWithContext, IntentInfo, IntentToFile,
...
use midnight_node_ledger_helpers::{DB, ProofKind, SignatureKind, UnshieldedWallet};
...
impl ContractDeployBuilder {
	pub fn new<D: DB + Clone>(args: ContractDeployArgs) -> Self {
		...
		if committee_seeds.is_empty() {
			committee_seeds = vec![Wallet::<D>::wallet_seed_decode(&funding_seed)];
		}
...
impl<D: DB + Clone> CreateIntentInfo<D> for ContractDeployBuilder {
	fn create_intent_info(&self) -> Box<dyn BuildIntent<D>> {
		println!("Create intent info for contract deploy");
		let deploy_contract: Box<dyn BuildContractAction<D>> = Box::new(ContractDeployInfo {
		...
		let actions: Vec<Box<dyn BuildContractAction<D>>> = vec![deploy_contract];
